...
export default async function IdeaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      ...
      <SectionL1 title={t('stakeholderEffects')}>
        <EffectsDiagram effectsDiagram={idea.effectsDiagram} stakeholderGroups={stakeholders} />
      </SectionL1>

      {/* Effects Chain Section */}
      <SectionL1 title={t('impactChain')}>

        {effects.length === 0 ? (
          <p className="text-muted-foreground">{t('noEffects')}</p>
        ) : (
                      ...
                      <div className="flex-1">
                        ...
                        {effect.mechanism && (
                          <div className="bg-muted p-3 rounded-md text-sm mb-4">
                            <div className="font-semibold mb-1">{t('mechanism')}</div>
                        ...
                        <div className="mt-4">
                          <div className="flex items-center gap-2 mb-2">
                            <Badge variant="outline" className="text-xs font-mono">{t('measurable')}</Badge>
                            <span className="font-medium">{effect.measurableTitle}</span>
                            <Badge variant="outline">
                              {t('unit', { unit: effect.measurableUnit })}
                            </Badge>
                          </div>
                          ...
                          <div key={gc.goalId} className="flex items-start gap-3 p-3 bg-muted/50 rounded-md">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <Badge variant="outline" className="text-xs font-mono">{t('goal')}</Badge>
                                <span className="font-medium">{gc.goalTitle}</span>
                              </div>
                              <div className="flex items-center gap-2 mb-1">
                                <Badge variant="outline" className="text-xs font-mono">{t('contribution')}</Badge>
                              ...
                              {gc.weight && (
                                <div className="text-xs text-muted-foreground mb-1">
                                  {t('weight', { weight: parseFloat(gc.weight).toFixed(1) })}
                                </div>
                              )}
                              {gc.goalMaslowLevel && (
                                <div className="text-xs text-muted-foreground mb-1">
                                  {t('maslowLevel', { level: gc.goalMaslowLevel })}
                                </div>
                              )}
      ...
      {/* Provisions Section */}
      <SectionL1 title={t('inspiredProvisions')}>

        {provisions.length === 0 ? (
          <p className="text-muted-foreground">{t('noProvisions')}</p>
        ) : (
              ...
              <Card key={provision.provisionId}>
                <CardHeader>
                  <div className="mb-2 flex gap-2">
                    <Badge variant="outline" className="text-xs font-mono">{t('provision')}</Badge>
                  </div>
