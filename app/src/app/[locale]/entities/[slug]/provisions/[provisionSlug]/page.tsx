...
export default async function ProvisionDetailPage({ params }: PageProps) {
  ...
  const td = await getTranslations('discussions')
    ...
    <div>
      ...
      {provision.console?.proposals &&
        provision.console.proposals.items.length > 0 && (
          <SectionL1 title={t('changeProposals')}>
      ...
      {linkedArtifacts.length > 0 && (
        <SectionL1 title={t('dataAndArtifacts')}>
      ...
      <SectionL1 title={t('performanceComparison')}>
        <div className="space-y-6">
          ...
          <div className="border border-dashed border-border rounded-lg p-4 bg-muted/20">
            <h4 className="text-sm font-medium mb-3 flex items-center gap-2">
              <span>⚖️</span> {t('theoryVsImplementation')}
            </h4>
            <p className="text-xs text-muted-foreground mb-3">
              {t('theoryDescription')}
            </p>
              ...
              <table className="text-xs w-full border-collapse">
                <thead>
                  <tr className="border-b border-border/50">
                    <th className="text-left py-2 px-2 font-medium">{t('aspect')}</th>
                    <th className="text-left py-2 px-2 font-medium">{t('theoryGeneral')}</th>
                    <th className="text-left py-2 px-2 font-medium">{t('thisImplementation')}</th>
                    <th className="text-center py-2 px-2 font-medium w-16">{t('rating')}</th>
                  </tr>
                </thead>
          ...
          <div className="border border-dashed border-border rounded-lg p-4 bg-muted/20">
            ...
            <div className="flex gap-4 opacity-50">
              <div className="flex-1 bg-green-50 rounded p-2 text-center">
                <div className="text-xs text-green-700">{t('benefits')}</div>
                <div className="text-sm text-green-800">-- {t('groups')}</div>
              </div>
              <div className="flex-1 bg-red-50 rounded p-2 text-center">
                <div className="text-xs text-red-700">{t('affected')}</div>
                <div className="text-sm text-red-800">-- {t('groups')}</div>
              </div>
            </div>
          </div>
          ...
          <div className="border border-dashed border-border rounded-lg p-4 bg-muted/20">
            ...
            <div className="grid grid-cols-2 gap-3 opacity-50">
              <div className="bg-background rounded p-2">
                <div className="text-xs text-muted-foreground">{t('annualBudget')}</div>
                <div className="text-sm font-medium">€--</div>
              </div>
              <div className="bg-background rounded p-2">
                <div className="text-xs text-muted-foreground">{t('revenueGenerated')}</div>
                <div className="text-sm font-medium">€--</div>
              </div>
            </div>
          </div>
          ...
          <div className="border border-dashed border-border rounded-lg p-4 bg-muted/20">
            <h4 className="text-sm font-medium mb-3 flex items-center gap-2">
              <span>🌍</span> {t('comparativeBenchmarking')}
            </h4>
            <p className="text-xs text-muted-foreground mb-3">
            ...
            <div className="flex flex-wrap gap-2 opacity-50">
              ...
              <span className="text-xs text-muted-foreground px-2 py-1">{t('more')}</span>
            </div>
          </div>
        </div>
      </SectionL1>
            ...
            {recentPosts.slice(0, 5).map((post) => (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {post.isPinned && <span className="text-xs text-muted-foreground">{td('pinned')}</span>}
                </div>
                <h4 className="text-sm font-medium">{post.title}</h4>
                <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                  <span>{td('votes', { count: post.score })}</span>
                  <span>{td('comments', { count: post.commentCount })}</span>
                  <span>{post.authorName || td('anonymous')}</span>
                </div>
              </Link>
            ))}
          </div>
