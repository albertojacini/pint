...
2. `GEMINI_API_KEY` environment variable (stored in `.env` at project root)

If neither is available, the script exits with an error message.

**Important:** Always source the env var before running: `export $(grep GEMINI_API_KEY .env) && uv run ...`
