"""Data models for the legislation research agent."""
...
class LegislationStatus(str, Enum):
    """Current legal status of legislation.
...
class Domain(str, Enum):
    """Domain/sector that the legislation regulates."""
    HOUSING = "housing"
    LAND_USE = "land_use"
...
class LegislationResearchInput(BaseModel):
    """Input for the legislation research agent."""
    description: str = Field(
        description="Description of the legislation to research (e.g., 'Milan congestion charge law')"
    )
...
class LegislationOutput(BaseModel):
    """Output legislation research object."""

    # Basic fields
    title: str = Field(
        description=(
            "Official name of the legislation in its original language. "
            "Should be clear and recognizable without bureaucratic jargon."
        )
    )
    description: str = Field(
        description=(
            "Explanation of what the legislation regulates and its main purpose. "
            "2-3 sentences covering the key aspects."
        )
    )
    status: LegislationStatus = Field(
        description="Current legal status of the legislation"
    )

    # Temporal validity
    effectiveFrom: str | None = Field(
        None,
        description="When the legislation took effect (YYYY-MM-DD format)"
    )
    effectiveUntil: str | None = Field(
        None,
        description="When the legislation ended/will end (YYYY-MM-DD format, null if still active)"
    )
...
class LegislationResearchError(BaseModel):
    """Error response when legislation cannot be researched."""
    error: str = Field(
        description=(
            "Error code: "
            "VAGUE_DESCRIPTION (cannot identify legislation), "
            "NOT_LEGISLATION (not a law/regulation), "
            "NOT_FOUND (no reliable information), "
            "MULTIPLE_MATCHES (ambiguous, multiple laws match)"
        )
    )
    reason: str = Field(
