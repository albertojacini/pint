/**
 * Events - BASE dataset
 * Key events demonstrating different types
 */

export const events = [
  {
    administration: "Milan City Council 2021-2026",
    title: "Area C Post-COVID Reactivation",
    description: "Area C enforcement resumes after COVID-19 suspension",
    descriptionShort: "Area C resumes post-COVID",
    type: "regulation_update",
    occurredAt: "2021-06-14T00:00:00Z"
  },
  {
    administration: "Milan City Council 2021-2026",
    title: "Area C Electric Vehicle Policy Update",
    description: "Extended exemptions for fully electric vehicles confirmed through 2030",
    descriptionShort: "EV exemptions extended",
    type: "regulation_update",
    occurredAt: "2023-06-15T00:00:00Z"
  },
]
